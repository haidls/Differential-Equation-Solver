...
def get_data(coefficient_value_amount, input_length, input_width, test_repetitions=1, test_data_amount=50):
    ...
    for i in range(0, test_data_amount):
        ...
        counter = counter + 1
        print("solved equation " + str(counter))
        testing_input.append(solution[0:-test_repetitions, :])
        testing_output.append(solution[-test_repetitions:, :])
    ...
    return numpy.vstack(training_input), numpy.array(training_output), \
           testing_input, testing_output, 0, 0 #t_eval_test, test_coeff


def solve_pde(coeff, grid, input_length):
    F = function(coeff)
    ...
    equation.solve(field, t_range=delta_time * input_length, dt=1e-1, tracker=storage.tracker(delta_time)).data
...
def function(coeff):
    return lambda t, s: coeff[0] * math.sin(2 * math.pi * s) + coeff[1] * s #+ coeff[1] * math.cos(2 * math.pi * s)


def function_string(coeff):
    return str(coeff[0]) + "*sin(2*pi*x)+" + str(coeff[1]) + "*x" #+ str(coeff[1]) + "*cos(2*pi*x)+"
